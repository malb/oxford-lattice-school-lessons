...
\section{BKZ}
\label{sec:org23e8eeb}
\begin{frame}[label={sec:org171853f}]{BKZ}
\begin{itemize}
...
\begin{frame}[label={sec:org09eae8e}]{BKZ 2.0}
\begin{description}
...
\begin{frame}[label={sec:org1565c93}]{root-Hermite Factors}
...
\begin{frame}[label={sec:orgb0862e4}]{Gaussian Heuristic}
...
\begin{frame}[label={sec:orgb383a22}]{Geometric Series Assumption}
...
\begin{frame}[label={sec:org50f573d}]{BKZ Quality}
...
\begin{frame}[label={sec:orgbf01d0e}]{BKZ Quality}
\begin{tikzpicture}
...
\begin{frame}[label={sec:org8cf4cb9}]{Running Time}
\begin{center}
...
\begin{frame}[label={sec:org9bf264a}]{Running Time}
Per tour, BKZ calls 
\begin{description}
...
\begin{frame}[label={sec:orgc617bab}]{Running Time}
We assume 
\begin{itemize}
...
\begin{frame}[label={sec:org650bac7}]{Asymptotic Behaviour}
...
\section{LWE}
\label{sec:org3d67e68}
\begin{frame}[label={sec:org5737bdf}]{Learning with Errors}
...
\label{sec:orgf427ccc}
\begin{frame}[label={sec:orgca5f381}]{Short Integer Solutions}
...
\begin{frame}[label={sec:orgf6555c5}]{Strategy}
...
\begin{frame}[label={sec:org15375f1}]{Required Quality}
...
\begin{frame}[label={sec:orga5726bd}]{Lattice Reduction}
...
\begin{frame}[label={sec:org3e6d2cc}]{Constructing a Basis}
\begin{itemize}
...
\begin{frame}[label={sec:orgb1433d1}]{Degrees of Freedom}
\begin{itemize}
...
\begin{frame}[label={sec:orgf3965fe}]{Choosing \(m\)}
...
\begin{frame}[label={sec:org11f0d25}]{Choosing \(ε\)}
\begin{tikzpicture}
\pgfplotsset{width=1.0\textwidth, height=0.5\textwidth}
...
\begin{frame}[label={sec:org87aa7e0}]{Choosing \(ε\)}
...
\begin{frame}[label={sec:org12a4d8b}]{Amortising Costs}
Producing \(1/ε^2\) short vectors is cheaper than \(1/ε^2\) calls to BKZ in block size \(β\).

Two options:

\begin{itemize}
\item Use that sieving outputs \(2^{0.2075\cdot β}\) vectors. \footfullcite{EPRINT:ADPS15}
...
\begin{frame}[label={sec:org4895355}]{LWE Normal Form}
...
\(\vec{a} ← \mathcal{U}(\ZZ_q^n)\), \(e ← χ\) and \alert{\(\vec{e}\)} such that all components \alert{\[e_i ← χ\]} in polynomial time.\footfullcite{C:ACPS09}
\end{block}
\end{frame}

\begin{frame}[label={sec:org6cbd55b}]{LWE Normal Form}
\begin{itemize}
...
\begin{frame}[label={sec:org4381bcb}]{Small Secret}
...
\begin{frame}[label={sec:orgd9cdd80}]{Honourable Mention: BKW}
\begin{columns}
...
\label{sec:org85686c7}
\begin{frame}[label={sec:orge66b3fc}]{Bounded Distance Decoding and unique SVP}
...
\begin{frame}[label={sec:orgfcf1293}]{Constructing a Basis}
\begin{itemize}
...
\begin{frame}[label={sec:org34886da}]{HSVP vs uSVP}
\begin{itemize}
...
\begin{frame}[label={sec:org880cce0}]{Success Condition (2008)}
...
\begin{frame}[label={sec:org0f97eb0}]{Success Condition (2013, 2016)}
\begin{itemize}
\item We can predict the length of the unusually short vector: \[λ_1(\vec{B}) ≈ \sqrt{m} ⋅ σ.\]

\item In general, we expect no other unusually short vectors, so we may assume \footfullcite{EPRINT:AlbFitGopf13,PhD:Goepfert16} \[λ_2(\vec{B}) ≈ \sqrt{\frac{d}{2\,π,e}} ⋅ \Vol(\vec{B})^{1/d}.\]
\end{itemize}
\end{frame}

\begin{frame}[label={sec:orgfaa7687}]{Success Condition (2015)}
...
\begin{frame}[label={sec:org5c285b8}]{Success Condition (2016)}
\begin{itemize}
...
\begin{frame}[label={sec:org8a16448}]{Success Condition (2016)}
\begin{itemize}
...
\begin{frame}[label={sec:orga45f051}]{Success Condition (2016)}
\begin{center}
\includegraphics[width=.9\linewidth]{./usvp-2016-visualisation.pdf}
\end{center}
\end{frame}

\begin{frame}[label={sec:org1c866bb}]{Comparison for \(q=2^{15}, σ=3.2\)}
\begin{center}
\includegraphics[width=.9\linewidth]{./usvp-comparison.pdf}
\end{center}
\end{frame}

\begin{frame}[label={sec:org452d475}]{LWE Normal Form}
\begin{itemize}
...
\begin{frame}[label={sec:org7b4f967}]{Small Secrets}
\begin{itemize}
...
\item Rescale the first part to have the same norm as the second.\footfullcite{ACISP:BaiGal14} 
\begin{itemize}
...
\begin{frame}[standout,label={sec:org99956ba}]{Fin}
\begin{center}
